...
#do imputation-
if dump_hist==1:
	tb.add_histogram('O2 Saturation  before imputation', torch.tensor(np.array(new_nc['nursingchartvalue'])))
print("Before Empty results are ",new_nc.isnull().sum())
#TODO:Need to check why it is 19
new_nc['nursingchartvalue'].fillna(value=98, inplace=True)
print("After Empty results are ",new_nc.isnull().sum())
if dump_hist==1:
	tb.add_histogram('O2 Saturation After imputation', torch.tensor(np.array(new_nc['nursingchartvalue'])))
...
for stayid in stayids:
    ...
    df.to_csv(os.path.join('o2_saturation', str(stayid) + '.csv'))

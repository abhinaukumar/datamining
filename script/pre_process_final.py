...
var_to_consider = ['glucose','O2 Saturation', 'Respiratory Rate','Temperature (C)']
...
def read_nc_table(eicu_path):
    ...
    typevallabel = ['O2 Saturation', 'Respiratory Rate']
    typevalname = ['Temperature (C)', 'Temperature (F)']
...
#            nclabpat = pd.merge(timeepisode, pat, on='patientunitstayid')
            df = binning(timeepisode, 60)
            df = imputer(df, strategy='normal')
            df.to_csv(os.path.join(t_path, merged, 'merged.csv'), index=False)
        except:
            continue
...
def binning(df, x=60):
    null_columns = ['glucose','O2 Saturation', 'Respiratory Rate', 'Temperature (C)']
...
parser.add_argument('--eicu_dir', help='Path to eICU database', required=True, type=str)
parser.add_argument('--output_dir', help='Output path', required=True, type=str)
args = parser.parse_args()

assert len(args.eicu_dir) > 0, 'Empty eicu db path'
assert len(args.output_dir) > 0, 'Empty output dir'
output_dir = os.path.join(args.output_dir)
lab_path = os.path.join(args.output_dir,'lab')
nc_path = os.path.join(args.output_dir,'nc')
dest_path = os.path.join(args.output_dir,'merged')
...
patients = pd.read_csv(os.path.join(args.eicu_dir, 'patient.csv'))
...
patients.to_csv(os.path.join(args.output_dir, 'filtered_patient.csv'))
del patients
...
#delete_wo_timeseries(args.output_dir)
#Write all the data into one dataframe
#all_df_into_one_df(args.output_dir)

for stayid in stayids:
	print(str(stayid))
	lab = dataframe_from_csv(os.path.join(args.output_dir,'lab', str(stayid) + '.csv'))
	nc = dataframe_from_csv(os.path.join(args.output_dir,'nc', str(stayid) + '.csv'))
	nclab = pd.concat([nc, lab]).sort_values(by=['itemoffset'])
	timeepisode = convert_events_to_timeseries(nclab, variables=var_to_consider)
	#nclabpat = pd.merge(timeepisode, pat, on='patientunitstayid')
	df = binning(timeepisode, 60)
	df = imputer(df, strategy='normal')
	df.to_csv(os.path.join(args.output_dir,'merged',str(stayid) + '.csv'), index=False)
